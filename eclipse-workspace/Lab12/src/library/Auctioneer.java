...
public class Auctioneer extends Agent {
	...
	protected void takeDown() {
		try {
			System.out.println("Terminating agent " + getAID().getLocalName() + " ...");
	...
	private void getInitialPrice() {
		...
		if (args != null && args.length >= 1) {
			try {
				...
				System.out.println("Add initial price");
				doDelete();
			}
		} else {
			System.out.println("No initial price, terminating agent");
			doDelete();
		}
	}
	...
	private class AuctioneerBehaviour extends FSMBehaviour {		
		...
		private List<AID> FinalParticipants;
		...
		private class SearchBuyersBehaviour extends OneShotBehaviour {
			...
			public void action() {
				...
				try {
					...
					if (result.length == 0) {
						System.out.println("Buyers not found");
					} else {
						System.out.println("Found possible buyers: ");
		...
		private class CallBuyersBehaviour extends OneShotBehaviour {
			...
			public void action() {
				...
				for (AID buyer : buyers) {
					...
					System.out.println("Send proposal to " + buyer.getLocalName());
				}
				myAgent.send(cfp);
			}
		}
			...
			public void action() {
				...
				if (cfpResponse != null) {
					...
					if (cfpResponse.getPerformative() == ACLMessage.PROPOSE) {
							...
							FinalParticipants.add (cfpResponse.getSender());
					...
					responsesReceived++;
					System.out.println("Buyer " + cfpResponse.getSender().getName() + "said: I  "
		...
		/*
		 * Informs the FinalParticipants of the auction and rejects all other proposals.
		 */
		private class AcceptProposalBehaviour extends OneShotBehaviour {
			...
			public void action() {
				...
				for(AID winner:FinalParticipants){
		 ...
		 * If the new price hits the reserve value, end auction without FinalParticipants.
		 */
		private class ReducePriceBehaviour extends OneShotBehaviour {			
			...
			public void action() {
				...
				System.out.println("No bids. Lowering price  " + currentPrice);
				System.out.println("Smallest price accepted is  " + reservePrice);
				
				if (currentPrice < reservePrice) {
					transitionStatus = belowReserve;
					System.out.println("Can't lower the price anymore. Finish auction");
				}
			}
		...
		private class EndingAuctionBehaviour extends OneShotBehaviour {
			...
			public void action() {
				...
				if (FinalParticipants != null) {
					for (AID EngP:FinalParticipants) {
							
							System.out.println("Auction finished, buyer is " + EngP.getName());
