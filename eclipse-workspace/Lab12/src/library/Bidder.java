	...
	private void getPriceToBuy() {
		...
		if (args != null && args.length >= 1) {
			try {
				...
				System.out.println("Please tell the system the price you want to buy the item for");
				doDelete();
			}
		} else {
			System.out.println("Terminating agent, missing buying price");
			doDelete();
		}
	}
	...
	private class BuyerBehaviour extends FSMBehaviour {
		...
		private class WaitAuctionBehaviour extends Behaviour {
			...
			public void action() {
				...
				if (inform != null) {
					System.out.println("Informed buyer  " + getAID().getLocalName());
					stopWaiting = true;
				} else {
					block();
				}
				...
				if (replyMessage != null) {
					...
					switch(replyMessage.getPerformative()) {
						case ACLMessage.ACCEPT_PROPOSAL:
							System.out.println(myAgent.getLocalName() + " I will buy the item");
							
							break;
						case ACLMessage.REJECT_PROPOSAL:
							System.out.println(myAgent.getLocalName() + " Oups, don't want it, sorry");
							break;
					}
				}
			}
